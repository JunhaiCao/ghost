...
def from_bin(filename, shape):
    ...
    def from_su(filename):
        """ Read .su file into a list of np.ndarrays """
        if not isinstance(filename, (list, tuple)):
            filename = [filename]
        outs = []
        for file in filename:
            with segyio.su.open(file, endian='little', ignore_geometry=True) as f:
                ...
                print(f'< {file} > np.array({outs[-1].shape})')
        return outs
...
def mkblcode(P, **kwargs):
    """
    TODO: make blending codes

    Parameters
    ----------------------------------------
    Inputs:
        "P"--------------------seismic Cube with shape (nrec,nsrc,nt)
        "blending_factor"------blending factor
        "space_pattern"--------blending pattern in space
        "time_pattern"---------time delay pattern
        "min_tau/max_tau"------min and max time delays
    Output:
        "code" --- blending codes 
    ----------------------------------------
    """
    ...
    # blended information
    blending_factor = optargin.get('blending_factor',2)
    space_pattern = optargin.get('space_pattern','max_distance')
    time_pattern = optargin.get('time_pattern','random_delay')
    min_tau= optargin.get('min_tau',0.1)
    max_tau = optargin.get('max_tau',2)
    dt = P.dt

    # generate firing pattern (space)
    num_src = P.data.shape[1]
    num_blsrc = int(blending_factor)
    num_exp = int(np.ceil(num_src/num_blsrc))

    if space_pattern == 'sequential':
        src_ids = np.arange(num_src)
        exp_ids = np.kron(np.arange(num_exp), np.ones((1, num_blsrc)))
        exp_ids = exp_ids[0:num_src]

    elif space_pattern == 'random_position':
        src_ids = np.random.shuffle(np.arange(num_src))
        exp_ids = np.kron(np.arange(num_exp), np.ones((1, num_blsrc)))
        exp_ids = exp_ids[0:num_src]

    elif space_pattern == 'max_distance':
        T = np.kron(np.ones((num_blsrc, 1)), np.eye(num_exp,dtype=int))
        T = T[:num_src, :]
        (posi,) = np.nonzero(T.flatten('F')) # find the position of the sources 
        (exp_ids , src_ids) = ind2sub(np.array([num_src,num_exp]),posi) 

    else:
        raise Exception('Error, space pattern not supported!')

    # generate firing pattern (time)
    if time_pattern == 'random_delay':
        tau = min_tau + (max_tau - min_tau)*np.random.rand(num_src, 1)
        tau = np.floor(tau/dt)*dt

    elif time_pattern == 'random_delay_panos':
        tmp_tau = 2*max_tau*np.random.rand(num_src, 1) - max_tau
        tau = np.zeros(tmp_tau.shape)
        for I in range(num_src):
            tau[I] = tmp_tau[src_ids[I]]
    elif time_pattern == 'user_time_pattern':
        pass # I will implenment this later 
    else:
        raise Exception('Time pattern not supported')
        
    # Export blending code
    code = {'blending_factor': blending_factor}
    code['num_blended_shots'] = num_exp
    code['source_location_ids'] = src_ids
    code['blended_shot_ids'] = exp_ids
    code['time_delays'] = tau
    code['nt'] = P.nt
    code['dt'] = dt
    code['sx'] = P.sx
    code['nsx'] = P.nsx
    code['nf'] = P.nf  #np.floor(P.shape[2]/2) + 1
...
class Blending():
    """
    Public methods:
        forward:        starts blending process
        adjoint:            starts pseudo-deblended process
    """
    ...
    def __init__(self, code, **kwargs):
        ...
        # data
        self.code = code
        self.G = Cube(np.zeros((code['nsx'], code['num_blended_shots'], code['nf']), dtype=complex), \
            fmt='xf', sx=np.arange(code['num_blended_shots']),rx=code['sx'], \
                dt=code['dt'], nt=code['nt'])

        upper_f = code['nf']
        df = 1/(code['nt']*code['dt'])
        
        for I in range(code['nsx']):
            self.G.data[code['source_location_ids'][I], code['blended_shot_ids'][I], 0:upper_f] = \
                np.exp(-1j*2.0*np.pi*np.arange(0,upper_f)*df*code['time_delays'][I])

        if is_normalize:
            self.G.data = self.G.data / np.sqrt(code['blending_factor'])
            
        self.GGH = self.G.mulcube(self.G.ctranspose())

        # methods
        self.forward = self.blend
        self.adjoint = self.pseudodeblend

    def blend(self, P):
        # check is fx domain
        if not P.is_xf():
            raise Exception('The input data must be in the fx domain for the blending operator.')
        
        # Pbl = P*Gammma
        D_out = P.mulcube(self.G)
        return D_out
    
    def pseudodeblend(self, P):
        # check is fx domain
        if not P.is_xf():
            raise Exception('The input data must be in the fx domain for the blending operator.')

        # Pps = Pbl*Gammma'
        D_out = P.mulcube(self.G.ctranspose())
